...
public class DTW {
   final static int DEFAULT_SEARCH_RADIUS = 3;
   ...
   private static TimeWarpInfo fastDTW(TimeSeries tsI, TimeSeries tsJ, int searchRadius, DistanceFunction distFn) {
      ...
      if ( (tsI.size() <= minTSsize) || (tsJ.size() <= minTSsize) ) { return getWarpInfoBetween(tsI, tsJ, distFn); } // perform full dynamic time warping
      else {
  ...
  public static TimeWarpInfo getWarpInfoBetween(TimeSeries tsI, TimeSeries tsJ, DistanceFunction distFn) { return DynamicTimeWarp(tsI, tsJ, distFn); }
  ...
  private static TimeWarpInfo DynamicTimeWarp(TimeSeries tsI, TimeSeries tsJ, DistanceFunction distFn) {
    ...
    for (int i=1; i<=maxI; i++) {  // i = columns
      // Calculate the value for the bottom row of the current column (i,0) = LocalCost(i,0) + GlobalCost(i-1,0)
  ...
  public static WarpPath getWarpPathBetween(TimeSeries tsI, TimeSeries tsJ, SearchWindow window, DistanceFunction distFn) { return constrainedTimeWarp(tsI, tsJ, window, distFn).getPath(); }
  public static TimeWarpInfo getWarpInfoBetween(TimeSeries tsI, TimeSeries tsJ, SearchWindow window, DistanceFunction distFn) { return constrainedTimeWarp(tsI, tsJ, window, distFn); }
