public class PAA extends TimeSeries { // PAA = Piecewise Aggregate Approximation https://jmotif.github.io/sax-vsm_site/morea/algorithm/PAA.html
   // PRIVATE DATA
   ...
   public PAA(TimeSeries ts, int shrunkSize) {
      ...
      // Ensures that the data structure storing the time series will not need to be expanded more than once.  (not necessary, for optimization)
   ...
   public int originalSize() { return originalLength; }
   public int aggregatePtSize(int ptIndex) { return aggPtSize[ptIndex]; }
   public String toString() { return "(" + this.originalLength + " point time series represented as " + this.size() + " points)\n" + super.toString(); }
}  // end class PAA