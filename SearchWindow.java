...
abstract public class SearchWindow {
   ...
   // Iterates through all cells in the search window in the order that Dynamic Time Warping needs to evaluate them. (first to last column (0..maxI) bottom up  (o..maxJ))
   public final Iterator iterator() { return new SearchWindowIterator(this); }
   ...
   private final void expandSearchWindow(int radius) {
      if (radius > 0) { // if radius <=0 then no search is necessary, use the current search window
         // Add all cells in the current Window to an array, iterating through the window and expanding the window at the same time is not possible because the window can't be changed during iteration through the cells.
         final ArrayList windowCells = new ArrayList(this.size());
         for (final Iterator cellIter=this.iterator(); cellIter.hasNext();) windowCells.add(cellIter.next());
