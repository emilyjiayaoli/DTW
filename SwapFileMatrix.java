...
class SwapFileMatrix implements CostMatrix {
   ...
   public void put(int col, int row, double value) {
      if ( (row<window.minJforI(col)) || (row>window.maxJforI(col)) ) throw new InternalError("CostMatrix is filled in a cell (col=" + col + ", row=" + row + ") that is not in the " + "search window");
            ...
            try {
               if (isSwapFileFreed) throw new InternalError("The SwapFileMatrix has been freeded by the freeMem() method");
               else {
         ...
         else throw new InternalError("A SwapFileMatrix can only fill in 2 adjacentcolumns at a time");
      }
   }
   ...
   // This method closes and delets the swap file when the object's finalize() mehtod is called.  This method will ONLY be called by the JVM if the object is garbage collected while the application is still running.
