        ...
        try {
            ...
            if (isLabeled) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if ((colToInclude.length == 0) || (Arrays.contains(colToInclude, currentCol))) labels.add(currentToken);
                    currentCol++;
                }  // end while loop

                // Make sure that the first column is labeled is for Time.
                if (labels.size() == 0) throw new InternalError("ERROR:  The first row must contain label " + "information, it is empty!");
                else if (!isFirstColTime) labels.add(0, "Time");
                else if (isFirstColTime && !((String) labels.get(0)).equalsIgnoreCase("Time")) throw new InternalError("ERROR:  The time column (1st col) in a time series must be labeled as 'Time', '" + labels.get(0) + "' was found instead");
            } else { // time series file is not labelled
            ...
            while ((line = br.readLine()) != null) {   // read lines until end of file
                if (line.length() > 0) { // ignore empty lines
                    ...
                    // Make sure that the current line has the correct number of currentLineValues in it.
                    //     if (st.countTokens() != (labels.size()+ignoredCol))
                    //         throw new InternalError("ERROR:  Line " + (tsArray.size()+1) + "contains the wrong number of currentLineValues. " + "expected:  " + (labels.size()+ignoredCol) + ", " + "found: " + st.countTokens());
                    ...
                    else timeReadings.add(new Double(timeReadings.size()));
...
//               timeValueMap.put(timeReadings.get(timeReadings.size()-1), readings);
        ...
        catch (IOException e) { throw new InternalError("ERROR:  Problem reading the file '" + inputFile + "'."); }  // end try block
    }  // end constructor
    ...
    private static char determineDelimiter(String filePath) {
        ...
        catch (IOException e) { return DEFAULT_DELIMITER; }
    ...
    public double getMeasurement(int pointIndex, int valueIndex) { return ((TimeSeriesPoint) tsArray.get(pointIndex)).get(valueIndex); }
    ...
    public double[] getMeasurementVector(int pointIndex) { return ((TimeSeriesPoint) tsArray.get(pointIndex)).toArray(); }
    public void setMeasurement(int pointIndex, int valueIndex, double newValue) { ((TimeSeriesPoint) tsArray.get(pointIndex)).set(valueIndex, newValue); }
